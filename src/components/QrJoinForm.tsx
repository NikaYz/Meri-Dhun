...
const QrJoinForm = () => {
  ...
  // Wrap joinOrganization in useCallback to prevent it from
  ...
  const joinOrganization = useCallback(
    ...
    // The dependency array tells useCallback when to recreate the function.
    // In this case, it only needs to be recreated if router or setError changes.
    // These values are stable, so the function will be stable too.
    [router, setError]
  );
  ...
  }, [joinOrganization]); // Now joinOrganization is a stable dependency
